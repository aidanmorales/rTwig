#' Import QSM
#'
#' @description Imports a QSM created by TreeQSM, Treegraph, AdQSM, AdTree,
#'  or SmartQSM
#'
#' @param filename a QSM file. Accepts `.mat` (TreeQSM, SmartQSM),
#'  `.json` (Treegraph), or `.obj` (AdQSM, AdTree)
#'
...
import_qsm <- function(filename) {
  ...
  # Ensure filename ends with a supported extension
  if (!extension %in% c("mat", "json", "obj")) {
    abort(
      message = "`filename` must end in `.mat`, `.json`, or `.obj`.",
  ...
  if (extension == "json") {
    ...
    ############################################################################
    ###### TreeQSM / SmartQSM (.mat) ###########################################
    ############################################################################

    # Dynamically check TreeQSM version
    required <- c(
      "Added", "Axe", "BAng", "BLen", "BOrd", "BPar", "BSeg", "BVol", "Base",
      "BoC", "CExt", "CPar", "FCB", "Len", "Rad", "SoC", "Sta", "TreeData"
    )

    tmp <- try(read_mat_flat(filename), silent = TRUE)
    ...
    if (!inherits(tmp, "try-error") && setequal(names(tmp), required)) {
      ...
      qsm <- tryCatch(
        read_mat_nested(filename),
        error = function(e) {
          abort(
            paste(
              "Failed to read QSM!",
              "i File is not a valid TreeQSM 2.0, 2.x.x, or SmartQSM structure.",
              sep = "\n"
            ),
            class = "data_format_error"
          )
      ...
      # Distinguish TreeQSM from SmartQSM
      if (!is.null(qsm$cylinder) && is.null(qsm$treedata$location)) {
        version <- "smartqsm"
      } else {
        version <- 2.3
      }
    ...
    if (version == 2.3) {
    ...
    } else if (version == "smartqsm") {
      ##########################################################################
      ###### SmartQSM Structure ################################################
      ##########################################################################
      inform("Importing SmartQSM")
      ...
      # Branch, Treedata, Rundata ----------------------------------------------
      branch <- mat_list_to_df(qsm$branch)
      treedata <- mat_list_to_df(qsm$treedata)
      rundata <- mat_list_to_df(qsm$rundata)
...
#' Converts a named MATLAB list to a data frame
#' @param x a named list from a MATLAB structure
#' @returns returns a data frame
#' @noRd
mat_list_to_df <- function(x) {
  nms <- names(x)
  if (is.null(nms) || length(x) == 0) {
    return(data.frame())
  }
...
#'
#' @description `import_treeqsm()` is deprecated and will be removed in a future
#'  version. Use `import_qsm()` instead.
#' @param ... function inputs
#' @return Returns a list
#' @export
import_treeqsm <- function(...) {
  warn(paste(
    "`import_treeqsm()` was deprecated in rTwig 1.5.0.",
    "i Please use `import_qsm()` instead.",
    sep = "\n"
  ))
  suppressMessages(import_qsm(...))
}

#' @title Import Treegraph
#'
#' @description `import_treegraph()` is deprecated and will be removed in a future
#'  version. Use `import_qsm()` instead.
#' @param ... function inputs
#' @return Returns a list
#' @export
import_treegraph <- function(...) {
  warn(paste(
    "`import_treegraph()` was deprecated in rTwig 1.5.0.",
    "i Please use `import_qsm()` instead.",
    sep = "\n"
  ))
  suppressMessages(import_qsm(...))
...
#' @title Import AdQSM
#'
#' @description `import_adqsm()` is deprecated and will be removed in a future
#'  version. Use `import_qsm()` instead.
#' @param ... function inputs
#' @return Returns a data frame
#' @export
import_adqsm <- function(...) {
  warn(paste(
    "`import_adqsm()` was deprecated in rTwig 1.5.0.",
    "i Please use `import_qsm()` instead.",
    sep = "\n"
  ))
  suppressMessages(import_qsm(...))
