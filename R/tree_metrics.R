...
branch_metrics <- function(cylinder, base) {
  ...
  cylinder %>%
    ...
    summarise(
      ...
      diameter_base_cm = first(.data$radius) * 200,
      diameter_mid_cm = case_when(n() > 1 ~ .data$radius[ceiling(n() / 2)] * 200, TRUE ~ .data$radius * 200),
      diameter_tip_cm = last(.data$radius) * 200,
