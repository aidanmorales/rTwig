...
inline double dot3(const V3& a, const V3& b){
  return a.x*b.x + a.y*b.y + a.z*b.z;
}

inline double nrm (const V3& a){
...
inline void centroid_radius(
  ...
  double sum=0;
  for(int t=0;t<k;++t){
    double dx=px[i0+t]-cx;
    double dy=py[i0+t]-cy;
    double dz=pz[i0+t]-cz;
...
inline bool is_circle(
  ...
  for(int t=0;t<k;++t)
    pbuf[t] = { px[i0+t], py[i0+t], pz[i0+t] };
  ...
  V3 u = sub(pbuf[i1], pbuf[0]);
  double un=nrm(u);
  if (un==0.0) return false;
  u={u.x/un,u.y/un,u.z/un};

  V3 a = sub(pbuf[i1], pbuf[0]);
  V3 b = sub(pbuf[i2], pbuf[0]);
  V3 n = {
    a.y*b.z - a.z*b.y,
    a.z*b.x - a.x*b.z,
    a.x*b.y - a.y*b.x
  ...
  double cx,cy;
  if (!circumcenter(qx[0],qy[0],
                    qx[i1],qy[i1],
                    qx[i2],qy[i2], cx, cy))
    return false;

  const double dx0 = qx[0]-cx;
  const double dy0 = qy[0]-cy;
  const double r2  = dx0*dx0 + dy0*dy0;
  const double thr = tol * (1.0 + std::fabs(r2));

  for(int t=0;t<k;++t){
    const double dx = qx[t]-cx;
    const double dy = qy[t]-cy;
...
inline void split_base(
  ...
  idxBottom.clear();
  idxTop.clear();
  idxBottom.reserve(facets);
  idxTop.reserve(facets);

  for (int rel = 0; rel < m; ++rel) {
    const int pos    = rel + 1;
    const int absIdx = i0 + rel;
    const bool toBottom =
      (pos == 1) || (pos == 2) || (pos >= 5 && (pos % 2 == 1));
    (toBottom ? idxBottom : idxTop).push_back(absIdx);
  }
}

// AdTree: branch ends at a ring of 10 facet points
// [[Rcpp::export]]
DataFrame build_adtree(
    DataFrame vertices,
    int facets = 10,
    double tol = 1e-6,
  ...
  while (i + base_pts <= n) {
    ...
    double tcx,tcy,tcz, tr;
    centroid_radius_idx(px,py,pz, idxBottom, bcx,bcy,bcz, br);
    centroid_radius_idx(px,py,pz, idxTop,    tcx,tcy,tcz, tr);
...

    {
      ...
      out_parent.push_back(parent_id);
    ...
    while (j + facets <= n) {
      if (!is_circle(px,py,pz, j, facets, tol, pbuf, qx, qy)) {
        // branch ends at previous ring; no single-point cylinder
        i = j;
        ++branch;
        branched = true;
        break;
...
// AdQSM: branch ends at a single point
// [[Rcpp::export]]
